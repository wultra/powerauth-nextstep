# Deploying Next Step on JBoss / Wildfly

Next Step is a backend service which manages operation steps and authentication methods. This chapter describes how to configure and deploy the Next Step application on JBoss / Wildfly.

## JBoss Deployment Descriptor

Next Step contains the following configuration in `jboss-deployment-structure.xml` file for JBoss / Wildfly:
```
<?xml version="1.0"?>
...
The deployment descriptor requires configuration of the `com.wultra.powerauth.nextstep.conf` module.

## JBoss Module for Next Step Configuration

Create a new module in `PATH_TO_JBOSS/modules/system/layers/base/com/wultra/powerauth/nextstep/conf/main`.
...
<configuration scan="true" scanPeriod="30 seconds">
        ...
        <property name="LOG_FILE_NAME" value="nextstep" />
...
Spring Boot running on WildFly or JBoss initializes [JtaTransactionManager](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/transaction/jta/JtaTransactionManager.html).

The Next Step Spring application uses the `ext` Spring profile which activates overriding of default properties by `application-ext.properties`.
