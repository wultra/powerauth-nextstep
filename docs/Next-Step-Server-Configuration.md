...
- `credential_gen_param` - parameters of the credential generating algorithm: `length`, `includeSmallLetters`, `smallLettersCount`, `includeCapitalLetters`, `capitalLettersCount`, `includeDigits`, `digitsCount`, `includeSpecialChars`, `specialCharsCount`, `specialChars` (optional set of special characters to use when `includeSpecialChars` is `true`; defaults to `^<>{};:.,~!?@#$%=&*[]()`)
